...
**MVP v0.1.0+** — 初始版本功能完备，P0端到端验证通过，P1测试覆盖已补齐。
...
### 测试覆盖
- [x] **185个单元/集成测试全部通过**（35原有 + 8端到端 + 92工具层 + 50core补充）
- [x] 端到端集成测试（test_e2e.py）：init→parse_spec→validate→save→list→run→results 完整流程
...
│   ├── spec_fetcher.py         # API规范发现和下载（trust_env=False）
...
tests/                         # 185个测试
├── test_e2e.py                # 端到端集成测试（8个，含mock HTTP server）
├── test_core/
